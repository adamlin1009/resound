...
function SearchField({ label, value, className = "" }: SearchFieldProps) {
  return (
...
function Search({}: Props) {
  ...
  return (
    ...
    >
      <SearchField label="Instrument" value={instrumentLabel} className="min-w-[140px]" />
      <span className="my-2 hidden w-px bg-rule sm:block" />
      <SearchField label="Location" value={locationLabel} className="hidden sm:flex min-w-[150px]" />
      <span className="my-2 hidden w-px bg-rule md:block" />
      <SearchField label="Window" value={durationLabel} className="hidden md:flex min-w-[120px]" />
      <span
        aria-hidden
